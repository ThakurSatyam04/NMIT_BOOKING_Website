...
const Equipments = ({ userDetails }) => {
    ...
    try {
      ...
      try {
        const updateResponse = await axios.put(
          `${APIURL}/api/equip/status/${equipid}`,
          {
  ...
  const handleEquipQuantity = async (selectedToTime, date) => {
    try {
      const getEquipSlots = await axios.get(
        `${APIURL}/api/equip/slots/equip/${equipid}`
      );
      ...
      if (date && fromTime && toTime) {
        if (clickToTime) {
          ...
          try {
            ...
            const timeSlot = await axios.put(
              `${APIURL}/api/equip/slots/${equipid}`,
              newTimeSlot
            );
  ...
  return isLoading ? (
    ...
    <div className="bg-blue-100">
          ...
          userDetails.userType == "SuperAdmin" ? (
            <div className="text-center md:text-left flex items-center justify-end mr-4 ">
              <button
                ...
                className="bg-[#75cce7] p-2 h-fit  rounded-md hover:brightness-90"
              >
                Booking Requests
              </button>
            </div>
                    ...
                    ? data.map((item) => {
                        return (
                          <EquipDetails
                            ...
                            labDetail={labDetail.email}
                            setEquipName={setEquipName}
                            setTotalQuantity={setTotalQuantity}
                            setIsChecked={setIsChecked}
                            clickToTime={clickToTime}
                            isChecked={isChecked}
                          />
                        );
                      })
      ...
      <div id="showCalender" className="h-[1px] bg-black mt-10"></div>
      {isChecked ? (
        <div
          className={`w-full flex items-center justify-center bg-blue-100 transition-opacity duration-500 ${
            isChecked ? "opacity-100" : "opacity-0 transform scale-9"
          }`}
        >
          <div
            id="date_time"
            className="w-8/12 flex flex-col md:flex-row justify-center items-center gap-20 bg-[#78c7df72] mt-10 rounded-xl shadow-md border-white border p-10"
          >
            <div className="flex flex-col mt-6">
              ...
              <div
                className={classNames(
                  "flex flex-col transition-opacity duration-500 ease-in-out opacity-100",
                  { hidden: !visibleCalender, "opacity-100": visibleCalender }
                )}
              >
                <Calendar
                  onChange={handleDate}
      ...
      <div className="text-center md:text-left flex justify-center pt-6 pb-4 items-center bg-blue-100">
        <button
          className="bg-[#75cce7] p-2 rounded-md hover:brightness-90"
